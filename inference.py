...
Output format (STRICTLY followed per spec):
    [START] task=<name> env=<env> model=<model>
...
# ---------------------------------------------------------------------------
# Clamp helper — guarantees score is strictly in (0, 1)
# ---------------------------------------------------------------------------
...
# Run ONE task per invocation (set via env var), defaulting to spam_detection
TASK_NAME = os.environ.get("TASK_NAME", "spam_detection")
ENV_NAME  = "EmailManagementEnv"


# ---------------------------------------------------------------------------
# Logging helpers — format must match spec EXACTLY
# reward/score: 2 decimal places
...
def log_step(
    ...
    action_clean = action.replace("\n", " ").replace("\r", "")
    print(
        f"[STEP] step={step} action={action_clean} reward={safe_r:.2f} "
        f"done={done_str} error={error_str}",
        flush=True,
    )
...
def log_end(success: bool, steps: int, score: float, rewards: List[float]) -> None:
    ...
    print(
        f"[END] success={str(success).lower()} steps={steps} "
        f"score={safe_score:.2f} rewards={reward_str}",
        flush=True,
    )
...
# ---------------------------------------------------------------------------
# Main — ONE task per run, ONE [START] + N [STEP] + ONE [END]
# ---------------------------------------------------------------------------
...
def main() -> None:
    ...
    all_rewards: List[float] = []
    steps_taken = 0
    score       = 0.01   # default: clamp-safe minimum
    success     = False
    ...
    try:
        ...
        # Compute final score
        if all_rewards:
            raw_mean = sum(all_rewards) / len(all_rewards)
        else:
            raw_mean = 0.01

        score   = clamp_score(raw_mean)
        # Use a meaningful success threshold: mean reward above 0.5 = success
        success = score > 0.50

    except Exception as exc:
        # Catch-all so [END] is always emitted
        score   = clamp_score(sum(all_rewards) / len(all_rewards) if all_rewards else 0.01)
        success = False
        # Print to stderr so it doesn't pollute stdout format
        print(f"[ERROR] Unhandled exception: {exc}", file=sys.stderr, flush=True)
